...
double Tcc(double cabalH2,double cabalO2,double TinH2, double TinO2,double delta,double Qlost,double Tcc){
    ...
    double v_O2exces=cabal_molarO2_exces/cabal_molarH2, v_H2exces=cabal_molarH2_exces/cabal_molarH2; 
    v_O2=0.5;
    v_H20=1;
    v_H2exces=0;
    v_O2exces=0;
    ...
    double num=v_H2*hH2+ v_O2*hO2- v_H20*entalpia_f_H2O- massa_molarH2*Qlost/cabalH2;
    while (error>delta)
    {   
        double Tcc_old=Tcc;
        ...
        Tcc=T0+(num)/(v_H20*H2O.cp+v_H2exces*H2.cp+v_O2exces*O2.cp);
        Tcc=Tcc_old+fr*(Tcc-Tcc_old);
        error=abs(Tcc_old-Tcc);
    }
    H2O.Propietats_termofisiquesH2O(T0,Tcc,1,1);
    std::cout<<T0-Tcc+num/(v_H20*H2O.cp)<<std::endl;
    return(Tcc);
}